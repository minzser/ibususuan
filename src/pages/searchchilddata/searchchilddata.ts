    ...
    this.mykid = this.items.map(items => items.mykid);
        ...
        for(let i = 0; i <= this.items.length; i++){
          if(this.mykidz == this.mykid[i]){
            ...
            this.items2.push({mykid: this.mykid[i], name: this.name[i], parent: this.parent[i], icparent: this.icparent[i], ibususuan: this.ibususuan[i], icibususuan : this.icibususuan[i]});
            break;
          }
      }
  }
